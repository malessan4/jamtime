...
export default function GroupClient({ groupId, groupName, dbUsers, serializedEvents }: GroupClientProps) {
  ...
  // Extraemos los colores que ya están siendo usados por otros integrantes
  const takenColors = dbUsers.map(u => u.color);
  
  // Seleccionamos por defecto el primer color que NO esté tomado
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.success && res.user) {
      ...
      // Ahora mostramos el error específico que manda el servidor (ej: color duplicado)
                ...
                {PRESET_COLORS.map((c) => {
                  ...
                  return (
                    <button
                      ...
                      `}
                    >
                      {/* Si está tomado, dibujamos una pequeña X o candado visual */}
